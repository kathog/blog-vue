...
<!-- comments -->
                    ...
                    <v-gravatar class="media-object" v-bind:email="comment0.email" alt="Nobody" default-img="mp"/>
									</div>
						...
						<div class="section-row">
							...
							<form class="post-reply" @submit.prevent="doSend" >
								<div class="row">
									...
									<div class="col-md-12">
										<div class="form-group">
											<textarea class="input" v-model="comment.message" placeholder="Wiadomość" required></textarea>
										</div>
  ...
  export default {
    ...
    data () {
      axios.get("/tagCloud")
        ...
        axios.post("/post/inc/" + this.$route.params.id)
        .then(response => {
        })
        .catch(e => {
          console.log(e)
...
axios.post("/mostViewed")
    ...
    methods: {
      ...
      deletePost () {
        ...
        axios.post("/post/delete/" + this.post._id.$oid, "", config)
          .then(response => {
            // console.log(response)
            this.$router.push("/");
          }).catch(e => {
      ...
      doSend: function (event) {
        let msg = "{\"email\": \""+ this.comment.email + "\", \"message\": \"" + this.comment.message + "\",\"username\":\"" + this.comment.username +"\",\"postId\":\""+ this.post._id.$oid +"\"}";
        console.log(msg);
        axios.post("/pushComment/" + this.post._id.$oid, msg)
          .then(response => {
            console.log(response.data);
            axios.get("/comments/" + this.post._id.$oid)
          .then(response => {
            this.comments = response.data
          })
          .catch(e => {
